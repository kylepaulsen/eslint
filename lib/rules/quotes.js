...
module.exports = {
    meta: {
        ...
        schema: [
            {
                enum: ["single", "double", "backtick", "consistent"]
            },
            {
                anyOf: [
                    {
    ...
    create(context) {
        ...
        let settings = QUOTE_SETTINGS[quoteOption || "double"];
        let avoidEscape = options && options.avoidEscape === true;

        var guessIntendedQuoteMark = function(ast) {
            var toVisit = [ast];
                    ...
                    for (var x = 0; x < len; x++) {
                        ...
                        if (!(/tokens|comment/i).test(key)) {
                            ...
                            if (childNode && (Object.prototype.toString.call(childNode) === "[object Array]" ||
                                ...
                                if (childNode.type === "Literal" && typeof childNode.value === "string") {
                                    var firstChar = childNode.raw.charAt(0);
                                    if (firstChar === "'") {
                                        return "single";
                                    } else if (firstChar === "\"") {
                                        return "double";
        ...
        if (quoteOption === "consistent") {
            var quoteType = guessIntendedQuoteMark(sourceCode.ast);
            settings = QUOTE_SETTINGS[quoteType || "double"];
        }
