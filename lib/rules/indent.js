...
module.exports = {
    ...
    create(context) {
        ...
        var guessIntendedIndentation = function(ast) {
            var toVisit = [ast];
            var foundBlockStatement = false;
            var blockStmtWSChars;
            while (toVisit.length) {
                var nextNode = toVisit.pop();
                if (typeof nextNode === "object") {
                    var keys = Object.keys(nextNode).reverse();
                    var len = keys.length;
                    for (var x = 0; x < len; x++) {
                        var key = keys[x];
                        if (!(/tokens|comment/i).test(key)) {
                            var childNode = nextNode[key];
                            if (childNode && (Object.prototype.toString.call(childNode) === "[object Array]" ||
                                childNode.type)) {

                                if (foundBlockStatement && childNode.loc) {
                                    var currentLine = sourceCode.lines[childNode.loc.start.line - 1];
                                    var whitespaceChars = (currentLine.match(/^( +|\t+)/) || [])[1];
                                    if (blockStmtWSChars) {
                                        // find the delta indentation.
                                        whitespaceChars = whitespaceChars.substring(blockStmtWSChars.length);
                                    }
                                    return whitespaceChars;
                                } else if (childNode.type === "BlockStatement" && childNode.body.length) {
                                    foundBlockStatement = true;
                                    var blockStatementLine = sourceCode.lines[childNode.loc.start.line - 1];
                                    blockStmtWSChars = (blockStatementLine.match(/^( +|\t+)/) || [])[1];
        ...
        if (context.options.length) {
            if (context.options[0] === "tab") {
                ...
                indentType = "space";
            } else if (context.options[0] === "consistent") {
                var guessedIndent = guessIntendedIndentation(sourceCode.ast);
                if (guessedIndent) {
                    if (guessedIndent.charAt(0) === " ") {
                        indentSize = guessedIndent.length;
                        indentType = "space";
                    } else {
                        indentSize = 1;
                        indentType = "tab";
